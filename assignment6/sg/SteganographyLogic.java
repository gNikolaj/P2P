package com.shpp.p2p.cs.ngrishchenko.assignment6.sg;
...
public class SteganographyLogic {
    ...
    public static boolean[][] findMessage(GImage source) {
        ...
        boolean[][] b = new boolean[(int) source.getHeight()][(int) source.getWidth()];

        for (int y = 0;  y < array.length; y++) {
            for (int x = 0; x < array[y].length; x++) {
                int red = GImage.getRed(array[y][x]);
                b[y][x] = red % 2 != 0;
            }
        }
        return b;
    ...
    public static GImage hideMessage(boolean[][] message, GImage source) {
        ...
        for (int y = 0; y < array.length; y++) {
            for (int x = 0; x < array[y].length; x++) {
                int red = GImage.getRed(array[y][x]);
                int green = GImage.getGreen(array[y][x]);
                int blue = GImage.getBlue(array[y][x]);

                if (message[y][x] && red %2 == 0) {
                    red++;
                    array[y][x] = GImage.createRGBPixel(red, green, blue);
                }
                if (!message[y][x] && red %2 != 0) {
                    red--;
                    array[y][x] = GImage.createRGBPixel(red, green, blue);
